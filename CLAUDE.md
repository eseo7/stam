...
**새 화면은 그리는 것이 아니라 기존 STAM 공통 컴포넌트를 조립한다.**
...
6. 필요한 컴포넌트가 없으면 임의 생성하지 말고 **작업을 멈추고 보고**한다.
7. **승인 없이 공통 CSS/JS를 확장하지 않는다.**
8. 화면 파일 안에 새 컴포넌트 정의(DOM 구조·클래스·스타일 블록)를 만들지 않는다.
9. **diff에 새 CSS/JS 파일이나 inline style/script가 있으면 작업 실패**로 본다.
...
STAM 화면·문서 작업을 시작하기 전 아래 파일을 **반드시** 읽고, 사용할 클래스/함수 목록을 먼저 보고한다.
