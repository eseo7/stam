 ...
 * No direct Firestore path construction. Register wiring: screen-spec-firestore-crud.js
 * ========================================================================== */
(function () {
  'use strict';
  ...
  function rowHtml(item) {
    ...
    return '<tr class="ss-sc-row stam-table-row ssv2-int-row" data-ss-firestore-id="' + esc(item.id) + '">' +
      ...
      '<td style="display:flex;align-items:center;gap:4px">' + imgChip(item.imageCount) + annChip(item.annotationCount) + '</td>' +
      '<td style="color:var(--t3);font-size:12px">' + esc(updated) + '</td>' +
  ...
  function updateFooter(shown, total) {
    ...
    if (shown === total) {
      ...
      tfoot.innerHTML = '필터 적용 · <b style="color:var(--t1)">' + shown + '</b>건';
    }
  }
