...
| 3 | `users/{uid}` upsert | create 또는 `updatedAt` 갱신 (후속 PR에서 write rules 개방) |
| 4 | Membership gate | `users/{uid}` read + `collectionGroup('members')` discovery |
| 5 | 라우팅 분기 | active ≥ 1 → `projects.html`; pending → `access-pending.html`; denied/removed/disabled → `access-denied.html`; 없음 → `no-project.html` |
| 6 | 프로젝트 목록 | active member 프로젝트만 카드 표시 |
| 7 | 프로젝트 생성 **또는** 선택 | 생성 시 `projects/{projectId}` + `members/{uid}` owner/active 동시 생성 |
...
| 단계 | read | write |
|------|------|-------|
| **현재** (단계 0) | 본인 `users/{uid}` only | **전면 deny** |
| **목표** (Auth 보강 PR) | 동일 | 본인 doc **create/update only** — 필드 whitelist (`email`, `emailNormalized`, `displayName`, `photoURL`, `provider`, `updatedAt`; `status` client 변경 금지) |

구현 방식 후보 (후속 PR에서 택일):

- **A.** controlled client write (rules field validation)
- **B.** Callable / Admin bootstrap on first login

1차 Gate에서는 **A 또는 B 중 하나를 선택**하되, 제품 화면에서 수동 Console seed에 의존하지 않는 것을 완료 조건으로 한다.
...
| **1** | 예정 | `users/{uid}` self bootstrap write |
| **2** | 예정 | `projects` create + creator `members/{uid}` owner/active |
| **3** | 예정 | `requirements` role-scoped write |
...
현재: **모든 write deny**. read는 active member + own membership discovery만 허용.
...
| **A2** | 프로젝트 생성 + owner member write rules + UI | P1 happy path 재현 |
| **1** | `requirements` write rules | emulator syntax |
| **1** | Requirements CRUD UI wiring | service contract |
...
- [ ] 프로젝트 선택 → Overview → 산출물 화면 `projectId` 유지
- [ ] membership gate 5분기 (P1–P8 beta matrix) PASS
- [ ] `test-auth-entry-flow-contract.mjs` PASS
- [ ] `test-project-context-guard-contract.mjs` PASS
...
**현재 상태 (main @ `a005a0d`):** Google Auth + membership read gate + project select 동작 중. `users/{uid}` client bootstrap · 프로젝트 생성 write · 산출물 write는 **미개방**. 본 문서는 후속 구현 PR의 기술 SSOT이다.
