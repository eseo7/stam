...
Firebase Hosting Preview 429는 Rules 실패와 다르다. Hosting channel quota 429는 비차단 인프라 이슈로 볼 수 있지만, Rules syntax check 미실행은 차단 조건이다.

---

## 5-8. Preview 429는 비차단이지만 원인 확인은 해야 한다
...
```txt
firebase-tools hosting:channel:deploy 단계에서 channel quota 429
RESOURCE_EXHAUSTED
기존 Preview channel quota 문제와 동일
```
