...
- Staging maintainer Google 세션 Firestore persistence QA: **미완료** (Cloud Agent — 세션·ADC 부재). 리포트 §9–§10; **PR #364 Draft 유지**.
- Playwright는 repo `package.json` **미추가** (임시 QA 런타임 only).

### 다시 열 조건

- maintainer §9 live persistence checklist 완료 → §10 PASS 갱신 후 **Ready** (PR #365 merge 후 Preview 재검증 포함).
- requirement delete rules + UI PR.
