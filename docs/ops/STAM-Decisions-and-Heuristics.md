...
- Staging maintainer Google 세션 Firestore persistence QA는 Cloud Agent 환경 제약으로 **본 PR 미실행** — 리포트 §9 checklist로 분리.
- Playwright는 repo `package.json` **미추가** (임시 QA 런타임 only).

### 다시 열 조건

- requirement delete rules + UI PR.
- staging maintainer live Firestore persistence QA (§9 checklist — 선택).
