...
| 항목 | 값 |
|------|-----|
| base | `main` @ `aa19e85` (FS-6B picker #378 + empty-list fix #380) |
| 선행 merge | FS-5 #374, FS-6A #377, FS-6B #378, picker race fix #380 |
...
### 7-2. FS-7 full checklist (staging live — 진행 중)
...
| 영역 | 판정 |
|------|------|
| Contract + shim smoke (§3) | **PASS** (main @ `aa19e85`) |
| PR #380 picker load (preview live) | **PASS** |
| Maintainer staging live full persistence | **진행 중** |
| Unlink 필드 실제 제거 (live) | **미확인 — PASS 금지** |

**FS-7 PR Ready 조건:** §5 staging live checklist W-01~W-12 완료 + §7-2 결과 기입 + W-07 Firestore Console 증빙.
