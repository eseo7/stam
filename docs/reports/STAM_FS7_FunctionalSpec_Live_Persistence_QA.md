...
### 7-3. Agent live checklist (GHA — run #29150397560)
...
#### PRECHECK-permission (BLOCKED-PERMISSION)

| Subcheck | 결과 |
|----------|------|
| `firestore-read-project` | **PASS** |
| `firestore-read-functional-specs` | **PASS** |
| `auth-custom-token` | **BLOCKED-PERMISSION** |
| `firestore-write-delete-probe` | **PASS** |

**IAM unblock 방향 (B안 승인 2026-07-12):** 기존 `FIREBASE_SERVICE_ACCOUNT_STAM_PREVIEW_HOSTING` 재사용. QA 전용 SA 보류.

| 승인된 최소 IAM | 적용 대상 |
|-----------------|-----------|
| `roles/firebaseauth.admin` | project `stam-preview-hosting` |
| `roles/iam.serviceAccountTokenCreator` | Hosting SA 리소스 (principal = 자기 자신) |

**금지:** Owner / Editor / Production / Secret·key 노출  
**상태:** Console IAM 적용 + GHA 재실행 **대기**

상세: `docs/reports/STAM_FS7_Live_QA_Permission_Precheck.md`

#### Writer / Viewer (미실행)

| ID | 결과 |
|----|------|
| W-01 ~ W-12 | **미실행** (permission precheck 차단) |
| W-10b | **미실행** |
| V-01 ~ V-03 | **미실행** |
| CLEANUP | **미실행** |

> **임의 PASS 없음.** Maintainer Console IAM 적용 후 GHA 재실행 → artifact로 §7-3 갱신.
...
| Agent live W-01~W-12 + W-10b | **BLOCKED-PERMISSION** — `auth-custom-token` only (run `29150397560`) |
...
**현재:** run `29150397560` — bootstrap PASS, **B안 IAM 승인** → Console 적용 + GHA 재실행 대기 → merge 보류
...
- `docs/reports/STAM_FS7_Live_QA_Permission_Precheck.md` — GHA run #29150397560 IAM 판정
