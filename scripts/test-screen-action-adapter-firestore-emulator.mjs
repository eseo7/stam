#!/usr/bin/env node
/**
 * STAM PR B — screenAction Adapter + Firestore compat emulator integration
 *
 ...
 *
 * Usage:
 *   node scripts/test-screen-action-adapter-firestore-emulator.mjs
 */
...
function runWithSpawnedEmulators() {
  let tempDir;
  try {
    tempDir = mkdtempSync(path.join(tmpdir(), 'stam-screen-action-adapter-emulator-'));
...
async function runAdapterFirestoreEmulatorTests() {
  ...
  await adminDb.doc(`projects/${APP_PROJECT}/screenSpecs/scr-adapter-target`).set({
    id: 'scr-adapter-target',
    projectId: APP_PROJECT,
    code: 'SCR-902',
  ...
  const editorEmail = 'sa-adapter-editor@test.local';
  try {
  ...
  const adapterSource = readFileSync(path.join(ROOT, 'stam/js/stam.screen-action-firestore-adapter.js'), 'utf8');
  ...
  vm.runInContext(adapterSource, vm.createContext({
    ...
    Error,
    Math,
  }), { filename: 'stam.screen-action-firestore-adapter.js' });

  const adapter = window.STAM.screenActionFirestoreAdapter.create();
  const txGets = [];
  ...
  await assert.rejects(
    () => adapter.create(APP_PROJECT, Object.assign({}, baseInput, { name: 'adapterAction' })),
    (err) => err.code === 'SCREEN_ACTION_DUPLICATE_NAME',
  );

  await adapter.update(APP_PROJECT, created.id, { actionType: 'navigate', targetScreenSpecId: 'scr-adapter-target', updatedBy: editorUid });
  const renamed = await adapter.update(APP_PROJECT, created.id, { name: 'adapterActionRenamed', updatedBy: editorUid });
  assert.equal(renamed.name, 'adapterActionRenamed');
  ...
  await assert.rejects(
    () => adapter.update(APP_PROJECT, second.id, { name: 'adapterActionRenamed', updatedBy: editorUid }),
    (err) => err.code === 'SCREEN_ACTION_DUPLICATE_NAME',
  );
  ...
  console.log('screen action adapter firestore emulator: PASS');
}
